...
<div {{ $attributes->only('class')->class(['mb-3']) }}>
    ...
    <textarea name="{{ $name }}" id="{{ $id }}" rows="{{ $rows }}" {{ $attributes->except('class')->class(['form-control', 'is-invalid' => $errors->has($name)]) }} @if ($required) required @endif>{{ old($name, $value) }}</textarea>
